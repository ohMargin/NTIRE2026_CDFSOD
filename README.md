...
As a strong starting point, we recommend ETS (https://github.com/jaychempan/ETS), the award-winning open-sourced solution from last year’s CD-FSOD Open Setting track, which provides a solid and competitive baseline for participants to build upon. In addition, we suggest Domain-RAG (https://github.com/LiYu0524/Domain-RAG), a plug-and-play method that can be readily integrated into existing pipelines to enhance cross-domain adaptation and few-shot detection performance with minimal overhead. Finally, for participants interested in high-performing solutions in the Closed Setting, we highlight X-Few (https://github.com/johnmaijer/X-Few-_CD-FSOD), the award-winning team solution from last year’s Closed Setting track, offering valuable insights and strong empirical performance under the closed-set evaluation protocol.
